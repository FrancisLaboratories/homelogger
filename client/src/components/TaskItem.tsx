...
function dueDateStyle(
  ...
  if (diffDays === 0)
    return { text: "Due today", className: "text-danger fw-semibold" };
  if (diffDays <= 7)
    return { text: `Due ${dueDate}`, className: "text-warning fw-semibold" };
...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${SERVER_URL}/task/delete/${task.id}`, {
        method: "DELETE",
      });
  ...
  const handleUncomplete = async () => {
    try {
      const res = await fetch(`${SERVER_URL}/task/uncomplete/${task.id}`, {
        method: "PUT",
      });
      if (!res.ok) throw new Error(await res.text());
