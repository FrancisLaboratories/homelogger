...
const ShowMaintenanceModal: React.FC<Props> = ({
  show,
  handleClose,
  maintenanceRecord,
    ...
    const loadAttachments = async () => {
      try {
        const resp = await fetch(
          `${SERVER_URL}/files/maintenance/${maintenanceRecord.id}`,
        );
        if (!resp.ok) return;
  ...
  const handleAddFiles = async (files: FileList | null) => {
    ...
    for (const f of Array.from(files)) {
      try {
        ...
        setAttachments((prev) => [
          ...prev,
          { id: uploadData.id, originalName: uploadData.originalName },
        ]);
      } catch (err) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(
        `${SERVER_URL}/maintenance/update/${maintenanceRecord.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${SERVER_URL}/maintenance/delete/${maintenanceRecord.id}`,
        { method: "DELETE" },
      );
  ...
  return (
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Footer
        className={`d-flex ${
          editing ? "justify-content-between" : "justify-content-end"
        }`}
      >
        {editing && (
