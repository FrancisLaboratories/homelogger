...
const TasksSection: React.FC<TasksSectionProps> = ({
  ...
  useEffect(() => {
    try {
      ...
      if (savedFilter && isFilterOption(savedFilter))
  ...
  const handleComplete = (updated: Task) => {
    ...
    if (
      !updated.isRecurring &&
      (filterOption === "upcoming" || filterOption === "overdue")
    ) {
  ...
  const filtered = tasks.filter((t) => {
    ...
    if (filterOption === "upcoming")
      return t.checked === false && !isOverdue(t);
    ...
    <div style={{ marginTop: "12px" }}>
      ...
      {overdueCount > 0 && filterOption === "upcoming" && (
        ...
        >
          ...
          {overdueCount} overdue task{overdueCount === 1 ? "" : "s"} — click to
          view
        </button>
      )}
